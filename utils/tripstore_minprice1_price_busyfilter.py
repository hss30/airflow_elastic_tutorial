...
def busy_filter():
    ...
    '''
    # 처음 했을 때:
    elasticsearch.helpers.reindex(es, "minprice1", "not_busy_list", target_client=es)
    # minprice1에 있는 모든 정보들을 복사해서 not_busy_list라는 인덱스를 새로 생성하여 복붙함
    # 하지만 이미 입력한 상황에서는 엘라스틱서치 에서 minprice1에 넣을 때 동시에 not_busy_list에 넣음
    '''
    for i in range(0,len(busy_list)):
        res = es.search(index="not_busy_list", doc_type='class',
                        body={"query": {"bool": {"must": [{"term": {"startdate": busy_list[i]}}]}}},
                        scroll='3m', size=10000)
        #res = es.search(index="not_busy_list", doc_type='class',
        #                body={"query": {"bool": {"must":[{"term":{"startdate":busy_list[i]}}, {"term":{"today":today}}]}}},
        #                scroll = '3m', size = 10000)
        # not_busy_list 안에 있는 정보들 중 출발 날짜가 성수기인 것들을 모조리 search 함
        for doc in res['hits']['hits']:
            # 찾은 서치들의 정보들을 doc 이라고 함
            res1 = es.get(index="not_busy_list", doc_type='class', id=doc['_id'])
            # 성수기 출발상품을 포함하는 아이디들을 호출
            doc1 = res1['_source']
            # 성수기 출발 상품들의 내용이 doc1
            res2 = es.index(index="busydays", doc_type='class', id=doc['_id'], body=doc1)
            # busydays라는 인덱스를 생성하고 동일한 id로 성수기 출발 상품들의 내용을 집어 넣음
            es.delete(index="not_busy_list", doc_type='class', id=doc['_id'])
    ...
    for i in range(len(today_list)):
        ...
        for j in range(delta.days +1):
            ...
            #res = es.search(index="not_busy_list", doc_type='class', body={
            #    "query": {"bool":{"must":[{"term": {"startdate": daylist[j]}}, {"term": {"today": today}}]}}}, scroll='3m',
            #                size=10000)
            res = es.search(index="not_busy_list", doc_type='class', body={
                "query": {"bool": {"must": [{"term": {"startdate": daylist[j]}}, {"term": {"today": today_list[i]}}]}}},
                            scroll='3m', size=10000)
