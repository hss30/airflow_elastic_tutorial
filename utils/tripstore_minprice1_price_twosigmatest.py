...
def minprice_ts_func_():
    ...
    for i in range(0, lnth):
        ...
        # i 번째의 여행장소 ID를 프린트함
        s = Search(using=es, index="not_busy_list", doc_type='class')
        ...
        upperlimit = meanprice + 2.5*stdprice
        lowerlimit = meanprice - 2.5*stdprice
        res = es.search(index="not_busy_list", doc_type='class', 
                        body={"query": {"bool": {"must": [{"term": {"cityId": str(city)}}, 
                                                          {"term": {"today": "2018-11-05"}}]}}}, 
        ...
        for doc in res['hits']['hits']:
            ...
            if todayprice < lowerlimit and float(todayprice) < 2/3 * meanprice:
                ...
                print(str(todayprice)+"&"+str(city))
        lenth = len(deletelist)
        for j in range(1, lenth):
            year = deletelist[j][8:12]
            month = deletelist[j][12:14]
            day = deletelist[j][14:16]
            print(str("https://m.tripstore.kr/search?placeId=")+str(city)+"&date="+str(year)+"-"+str(month)+"-"+str(day))
            print(str("https://m.tripstore.kr/calendar?path=search&cityId=1&placeId=") + str(city))