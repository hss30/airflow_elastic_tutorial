...
def airtel_ts_func_():
    ...
    for i in range(0, lnth):
        ...
        # i 번째의 여행장소 ID를 프린트함
        s = Search(using=es, index="not_busy_list_airtel", doc_type='class')
        ...
        upperlimit = meanprice + 2*stdprice
        lowerlimit = meanprice - 2*stdprice
        res = es.search(index="not_busy_list_airtel", doc_type='class', body={
            "query": {"bool": {"must": [{"term": {"cityId": str(city)}}, {"term": {"today": '2018-11-07'}}]}}}, scroll='3m',
                        size=10000)
        ...
        for j in range(1, lenth):
            ...
            print(str("https://m.tripstore.kr/search?placeId=") + str(city) + "&date=" + str(year) + "-" + str(
                month) + "-" + str(day))
