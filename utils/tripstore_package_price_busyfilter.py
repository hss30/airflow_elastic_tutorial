...
def package_busy_filter():
    ...
    '''
    # 처음 했을 때:
    elasticsearch.helpers.reindex(es, "package", "not_busy_list_package", target_client=es)
    # package에 있는 모든 정보들을 복사해서 not_busy_list_package라는 인덱스를 새로 생성하여 복붙함
    # 하지만 이미 입력한 상황에서는 엘라스틱서치 에서 package에 넣을 때 동시에 not_busy_list_package에 넣음
    '''
    ...
    #for j in range(0, len(today_list)):
        ...
        for doc in res['hits']['hits']:
            ...
            res1 = es.get(index="not_busy_list_package", doc_type='class', id=doc['_id'])
            # 성수기 출발상품을 포함하는 아이디들을 호출
            doc1 = res1['_source']
            # 성수기 출발 상품들의 내용이 doc1
            res2 = es.index(index="busydays_package", doc_type='class', id=doc['_id'], body=doc1)
            # busydays라는 인덱스를 생성하고 동일한 id로 성수기 출발 상품들의 내용을 집어 넣음
            es.delete(index="not_busy_list_package", doc_type='class', id=doc['_id'])
